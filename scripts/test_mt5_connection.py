...
try:
    ...
    print(
        "⚠️  Screenshot dependencies not available. Install with: pip install pyautogui pygetwindow pillow"
    )
...
def test_mt5_import():
    ...
    try:
        ...
        print(
            f"   Version: {mt5.__version__ if hasattr(mt5, '__version__') else 'Unknown'}"
        )
        return mt5
    ...
    try:
        ...
        for symbol in symbols:
            if symbol.name == "EURUSD":
                ...
                "USD" in symbol.name and len(symbol.name) <= 6
            ):  # Standard forex pair length
        ...
        if rates is not None and len(rates) > 0:
            ...
            latest = rates[-1]
            print(
                f"     Latest: O={latest['open']:.5f} H={latest['high']:.5f} L={latest['low']:.5f} C={latest['close']:.5f}"
            )
        ...
        try:
            ...
            rates_range = mt5.copy_rates_range(
                test_symbol, mt5.TIMEFRAME_M1, from_date, to_date
            )
        ...
        try:
            ...
            if ticks is not None and len(ticks) > 0:
                ...
                latest_tick = ticks[-1]
                print(
                    f"     Latest tick: {datetime.fromtimestamp(latest_tick['time'])} Bid={latest_tick['bid']:.5f} Ask={latest_tick['ask']:.5f}"
                )
            else:
        ...
        try:
            ...
            ticks_range = mt5.copy_ticks_range(
                test_symbol, from_date, to_date, mt5.COPY_TICKS_ALL
            )
    ...
    try:
        ...
        if positions:
            ...
            for pos in positions[:3]:  # Show first 3 positions
                print(
                    f"     {pos.symbol}: {pos.type} {pos.volume} lots, Profit: {pos.profit:.2f}"
                )
        ...
        if orders:
            ...
            for order in orders[:3]:  # Show first 3 orders
                print(
                    f"     {order.symbol}: {order.type} {order.volume} lots @ {order.price_open}"
                )
...
def create_test_chart_image(
    width: int = 800, height: int = 600, symbol: str = "EURUSD"
) -> bytes:
    ...
    try:
        ...
        # Add price axis
        draw.line(
            [(width - 80, 60), (width - 80, height - 60)], fill=(100, 100, 100), width=2
        )
        draw.line(
        ...
        for i, price in enumerate(prices[1:], 1):
            ...
            open_y = (
                height
                - 60
                - int(((open_price - min_price) / price_range) * chart_height)
            )
            close_y = (
                height
                - 60
                - int(((close_price - min_price) / price_range) * chart_height)
            )
            high_y = (
                height
                - 60
                - int(((high_price - min_price) / price_range) * chart_height)
            )
            low_y = (
                height
                - 60
                - int(((low_price - min_price) / price_range) * chart_height)
            )
    ...
    try:
        ...
        if captured_screenshots:
            ...
            for symbol, data in captured_screenshots.items():
                filename = (
                    screenshots_dir
                    / f"mt5_{symbol}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
                )
                ...
                if signal_result:
                    ...
                    try:
                        ...
                        json_match = re.search(
                            r"```(?:json)?\s*({.*?})\s*```", signal_result, re.DOTALL
                        )
                        if json_match:
                        ...
                        print(
                            f"       Confidence: {parsed_signal.get('confidence', 'N/A')}"
                        )
            ...
            for prompt_config in analysis_prompts:
                try:
                    ...
                    if result and len(result) > 100:
                        ...
                        print(
                            f"     {prompt_config['name']}: Success ✅ ({len(result)} chars)"
                        )
                        ...
                        print(
                            f"     {prompt_config['name']}: Failed ❌ (short response)"
                        )
...
{{
    ...
    "reasoning": "brief explanation"
                ...
                if signal_result:
                    ...
                    try:
                        ...
                        if json_str:
                            ...
                            print(
                                f"       Symbol: {parsed_signal.get('symbol', 'N/A')}"
                            )
                            print(f"       Bias: {parsed_signal.get('bias', 'N/A')}")
                            print(
                                f"       Action: {parsed_signal.get('action', 'N/A')}"
                            )
                            print(
                                f"       Confidence: {parsed_signal.get('confidence', 'N/A')}%"
                            )
                            print(
                                f"       Entry: {parsed_signal.get('entry_price', 'N/A')}"
                            )
                            print(
                                f"       Stop Loss: {parsed_signal.get('stop_loss', 'N/A')}"
                            )
                            print(
                                f"       Take Profit: {parsed_signal.get('take_profit', 'N/A')}"
                            )
                            print(
                                f"       R:R Ratio: {parsed_signal.get('risk_reward_ratio', 'N/A')}"
                            )
                        else:
        ...
        if total_analyses > 0:
            ...
            results_file = (
                results_dir
                / f"mt5_analysis_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
            )
    ...
    try:
        ...
        try:
            ...
            real_signal = await analyzer.analyze(
                screenshot_data=test_image_data, market_context=market_context
            )
        ...
        if ai_signal:
            ...
            try:
                ...
                if ai_signal.get("setups"):
                    ...
                    entry_price = (
                        setup["entry_zone"][0]
                        if isinstance(setup["entry_zone"], list)
                        else setup["entry_zone"]
                    )
                    sl_price = setup["sl"]
    ...
    try:
        ...
        try:
            ...
            if rr_ratio >= 1.5 and confidence >= 60:
                print(
                    f"   Signal validation: Success ✅ (RR: {rr_ratio:.2f}, Confidence: {confidence}%)"
                )
            else:
                print(
                    f"   Signal validation: Warning ⚠️ (RR: {rr_ratio:.2f}, Confidence: {confidence}%)"
                )
        ...
        if sl_distance_points > 0 and point_value > 0:
            position_size = max_risk_amount / (
                sl_distance_points * point_value / 100000
            )
            position_size = max(
                symbol_info.volume_min, min(position_size, symbol_info.volume_max)
            )
        else:
        ...
        missing_fields = [
            field for field in required_fields if field not in order_request
        ]
...
def test_config():
    ...
    try:
        ...
        try:
            ...
            print(
                f"   MT5 configured in app: {'Yes ✅' if config.mt5.is_configured else 'No ❌'}"
            )
        except Exception as e:
