...
async def test_platform_manager():
    ...
    try:
        ...
        for platform, success in connection_results.items():
            ...
            print(
                f"   {status} {platform.upper()}: {'Connected' if success else 'Failed'}"
            )
...
async def test_mock_trading():
    ...
    try:
        ...
        print(
            f"     Connected: {status['connected_platforms']}/{status['total_platforms']}"
        )
...
async def main():
    ...
    if platform_manager and has_connections:
        ...
        print(
            f"🔗 Connected Platforms: {', '.join(platform_manager.get_connected_platforms())}"
        )
