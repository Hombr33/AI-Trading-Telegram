...
async def test_openai_config():
    ...
    try:
        ...
        print(
            f"     General analysis: {'✅' if analyzer.validate_model_for_analysis('general') else '❌'}"
        )
        print(
            f"     Image analysis: {'✅' if analyzer.validate_model_for_analysis('image') else '❌'}"
        )
        print(
            f"     Real-time data: {'✅' if analyzer.validate_model_for_analysis('realtime') else '❌'}"
        )
        print(
            f"     Web search: {'✅' if analyzer.validate_model_for_analysis('web_search') else '❌'}"
        )
        print(
            f"     Advanced reasoning: {'✅' if analyzer.validate_model_for_analysis('reasoning') else '❌'}"
        )
