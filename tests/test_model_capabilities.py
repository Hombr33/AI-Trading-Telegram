...
def test_model_capabilities():
    ...
    try:
        ...
        print("-" * 80)
        print(f"{'Model':<15} {'Web Search':<12} {'Real-time':<12} {'Vision':<8} {'Reasoning':<10}")
        print("-" * 80)
...
def test_configuration():
    ...
    try:
        ...
        if settings_path.exists():
            ...
            print(f"   Web search enabled: {openai_config.get('web_search_enabled', 'Not set')}")
            print(f"   Real-time data enabled: {openai_config.get('realtime_data_enabled', 'Not set')}")
        else:
