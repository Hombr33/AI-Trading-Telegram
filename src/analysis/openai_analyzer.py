...
class OpenAIAnalyzer(IAnalyzer):
    ...
    def _validate_model_setup(self):
        ...
        try:
            ...
            logger.info(f"  - Web Search: {'✅' if capabilities.get('web_search') else '❌'}")
            logger.info(f"  - Real-time Data: {'✅' if capabilities.get('realtime_data') else '❌'}")
            logger.info(f"  - Vision: {'✅' if capabilities.get('vision') else '❌'}")
            logger.info(f"  - Advanced Reasoning: {'✅' if capabilities.get('reasoning') else '❌'}")
    ...
    def _get_model_recommendations(self, capabilities: Dict[str, bool]) -> List[str]:
        ...
        if not capabilities.get('web_search'):
            recommendations.append("Consider upgrading to gpt-4o-mini, gpt-4o, or gpt-5 for web search capabilities")
        
        if not capabilities.get('realtime_data'):
            recommendations.append("Consider upgrading to gpt-4o-mini, gpt-4o, or gpt-5 for real-time data access")
        
        if not capabilities.get('vision'):
            recommendations.append("Consider upgrading to gpt-4o or gpt-5 for image analysis capabilities")
        
        if not capabilities.get('reasoning'):
            recommendations.append("Consider upgrading to gpt-5 for advanced reasoning capabilities")
    ...
    async def analyze(self, 
        ...
        try:
            ...
            if not self.validate_model_for_analysis(analysis_type):
                logger.warning(f"Model {self.model} does not support {analysis_type} analysis")
                return self._get_fallback_response(market_context, "Model capability not supported")
            
            # Get REAL market data (not fake data)
            symbols = market_context.get("symbols", [])
            if not symbols:
                return self._get_fallback_response(market_context, "No symbols specified")
            
            logger.info(f"Starting analysis for symbols: {symbols}")
            
            # Fetch REAL market data using the new provider
            real_market_data = await self.realtime_provider.get_current_market_data(symbols)
            ...
            if self._is_fake_data(real_market_data):
                ...
                return self._get_fallback_response(market_context, "Real data provider failed")
            
            # Create analysis prompt with REAL data
            analysis_prompt = self.prompt_manager.create_analysis_prompt(
                market_context=market_context,
                realtime_data=real_market_data
            )
    ...
    def _is_fake_data(self, market_data: str) -> bool:
        ...
        fake_indicators = [
            "mock", "fake", "sample", "testing", "placeholder",
            "august 24, 2025", "2025-08-24",  # Future dates
            "29,450", "30,200", "28,750",  # Wrong BTC prices
            "35,750", "36,500"  # Wrong BTC prices
        ]
        
        data_lower = market_data.lower()
        fake_indicators_found = sum(1 for indicator in fake_indicators if indicator in data_lower)
        
        # If we find multiple fake indicators, it's likely fake data
        return fake_indicators_found >= 2
    
    async def _generate_signal_with_real_data(self, 
                                            analysis_prompt: str,
    ...
    async def get_economic_calendar(self, timeframe: str = "today", 
        ...
        try:
            return await self.realtime_provider.get_economic_calendar(timeframe, impact_level)
        except Exception as e:
