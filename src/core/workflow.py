...
class Component:
    ...
    async def restart(self) -> bool:
        ...
        log_system_event(self.name, "restarting", f"Restarting component (attempt {self.restart_count})")
    ...
    async def health_check(self) -> bool:
        ...
        try:
            ...
            log_error_with_context(e, {"component": self.name, "operation": "health_check"})
            return False
...
class WorkflowManager:
    ...
    async def startup(self) -> bool:
        ...
        for component_name in self.startup_order:
            ...
            try:
                ...
                log_operation_timing(f"startup_{component_name}", component_start, time.time())
        ...
        if failed_components:
            log_system_event("workflow", "startup_partial", 
                           f"Startup completed with failures: {failed_components}")
            return False
        else:
            log_system_event("workflow", "startup_complete", 
    ...
    async def shutdown(self) -> bool:
        ...
        log_system_event("workflow", "shutdown_begin", "Shutting down application components")
        ...
        if failed_components:
            log_system_event("workflow", "shutdown_partial",
                           f"Shutdown completed with failures: {failed_components}")
            return False
        else:
            log_system_event("workflow", "shutdown_complete",
            ...
            try:
                ...
                for component_name, component in self.components.items():
                    try:
                        ...
                        if not is_healthy and component.status == ComponentStatus.RUNNING:
                            logger.warning(f"Component {component_name} health check failed")
                            
                            # Attempt restart if within limits
                            if component.restart_count < component.max_restarts:
                                logger.info(f"Attempting to restart unhealthy component: {component_name}")
