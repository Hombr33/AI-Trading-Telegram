...
from .core.logging import (
    get_logger, log_system_event, print_banner, 
    print_status_table, log_error_with_context
)
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        log_system_event("main", "startup", "AI Trading Bot application started successfully")
        
        # Print status table
        status_data = {
            "MT5 Executor": {"status": "initialized", "details": "Ready for connection"},
        ...
        try:
            ...
            log_system_event("main", "shutdown", "AI Trading Bot application shut down successfully")
...
async def root():
    ...
    return {
        ...
        "components": {
            ...
            "socketio_bridge": socketio_bridge.get_status() if socketio_bridge else {"connected": False},
            "position_manager": position_manager.is_running if position_manager else False,
            "trailing_manager": trailing_manager.is_running if trailing_manager else False,
    ...
    if mt5_executor:
        try:
            ...
            status["components"]["mt5"] = {
                "connected": connected,
                "account_info": await mt5_executor.get_account_info() if connected else None
            }
        except Exception as e:
            status["components"]["mt5"] = {
                "connected": False,
...
async def get_config():
    ...
    return {
        ...
        "database": {
            "url": str(config.database.url).replace(config.database.password, "***") if config.database.password else str(config.database.url)
        },
        "trading": {
