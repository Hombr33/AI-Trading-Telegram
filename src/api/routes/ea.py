...
class EACloseRequest(BaseModel):
    ...
    api_key: str = Field(..., description="EA API key")
    ticket: int = Field(..., description="Position ticket")
    volume: Optional[float] = Field(None, description="Volume to close (None for full close)")
...
async def ea_order(request: EAOrderRequest):
    ...
    try:
        ...
        if result and result.get("success"):
            ...
            logger.info(f"Order executed successfully for user {user_id}, ticket: {ticket}")
            ...
            return EAOrderResponse(
                success=True,
                ticket=str(ticket)
            )
        else:
            error_msg = result.get("error", "Unknown error") if result else "Order execution failed"
        ...
        try:
            ...
            for pos in db_positions:
                positions.append({
                    ...
                    "time_open": pos.time_open.isoformat() if pos.time_open else None
                })
    ...
    try:
        ...
        try:
            ...
            if order_manager and hasattr(order_manager, 'platform_manager'):
                try:
                    platform_status = order_manager.platform_manager.get_platform_status()
                    if platform_status.get("connected_platforms", 0) > 0:
                        ...
                        if primary_platform and hasattr(primary_platform, 'get_account_info'):
        ...
        try:
            ...
            if success:
                ...
                try:
                    ...
                    logger.warning(f"Failed to send modification notification: {notify_error}")

                return EAModifyResponse(success=True)
            else:
                return EAModifyResponse(
                    success=False,
                    error="Position modification failed"
                )
...
async def ea_trade_history(request: EAHistoryRequest):
    ...
    try:
        ...
        try:
            ...
            db_trades = db_session.query(Trade).filter(
                Trade.user_id == user_id,
                Trade.time_close >= cutoff_date
            ).order_by(Trade.time_close.desc()).all()
            ...
            for trade in db_trades:
                trades.append({
                    ...
                    "time_open": trade.time_open.isoformat() if trade.time_open else None,
                    "time_close": trade.time_close.isoformat() if trade.time_close else None
                })
...
async def ea_update_settings(request: EASettingsRequest):
    ...
    try:
        ...
        try:
            success = await config_manager.update_user_config(user_id, "ea_settings", request.settings)
