...
class PlatformConnectionCreateRequest(BaseModel):
    ...
    test_connection: Optional[bool] = Field(
        True, description="Test connection on creation"
    )
...
class ConfigurationTemplateRequest(BaseModel):
    ...
    template_name: str = Field(
        ..., description="Template name (conservative/aggressive/scalping)"
    )
...
class SignalRequest(BaseModel):
    ...
    confidence: int = Field(
        ..., ge=0, le=100, description="Signal confidence percentage"
    )
...
class SignalDistributionRequest(BaseModel):
    ...
    distribution_type: str = Field(
        ..., description="Distribution type (immediate/delayed/batch)"
    )
...
class SignalSubscriptionRequest(BaseModel):
    ...
    min_confidence: int = Field(
        60, ge=0, le=100, description="Minimum confidence threshold"
    )
...
async def get_all_users(
    ...
    subscription_filter: Optional[SubscriptionStatusEnum] = Query(
    ...
    try:
        ...
        for user in users:
            ...
            if (
                subscription_filter
                and user["subscription_status"] != subscription_filter.value
            ):
                continue
...
async def register_platform_connection(
    ...
    try:
        ...
        platform_type = (
            PlatformType.MT5
            if request.platform_type == PlatformTypeEnum.MT5
            else PlatformType.CRYPTO
        )
        ...
        return PlatformConnectionResponse(
            ...
            api_key_masked=(
                new_connection["api_key"][:8] + "..."
                if new_connection["api_key"]
                else None
            ),
...
@router.get(
    "/users/{telegram_id}/connections", response_model=PlatformConnectionsListResponse
)
...
@router.put(
    "/users/connections/{connection_id}", response_model=PlatformConnectionResponse
)
...
@router.get(
    "/users/{telegram_id}/configuration", response_model=ConfigurationListResponse
)
async def get_user_configuration(
    telegram_id: int = Path(..., description="Telegram user ID"),
    config_type: Optional[ConfigTypeEnum] = Query(
        None, description="Specific configuration type"
    ),
    service: MultiUserService = Depends(get_multi_user_service),
    ...
    try:
        ...
        return ConfigurationListResponse(
            configurations=configurations, total_count=len(configurations)
        )
...
async def backup_user_configuration(
    ...
    try:
        backup_data = await service.config_manager.backup_user_configs(
            request.telegram_id
        )
...
@router.post(
    "/users/{telegram_id}/signal-subscription",
...
@router.get(
    "/users/{telegram_id}/signal-subscriptions",
    response_model=SignalSubscriptionsListResponse,
)
...
async def unsubscribe_from_symbol(
    ...
    try:
        success = await service.user_manager.unsubscribe_from_symbol(
            telegram_id, symbol
        )
    ...
    try:
        ...
        return TradingStatusResponse(
            ...
            platform_connections=(
                status_data.get("ea_connection", "").split(",")
                if status_data.get("ea_connection")
                else []
            ),
...
async def modify_user_position(
    ...
    try:
        result = await service.modify_user_position(
            telegram_id, ticket, request.sl, request.tp
        )
...
async def close_user_position(
    telegram_id: int = Path(..., description="Telegram user ID"),
    ticket: int = Path(..., description="Position ticket"),
    volume: Optional[float] = Query(
        None, description="Volume to close (partial close)"
    ),
    service: MultiUserService = Depends(get_multi_user_service),
) -> Dict[str, Any]:
...
async def demote_user_from_admin(
    ...
    try:
        success = await service.user_manager.remove_admin(
...
async def get_service_stats(
    ...
    try:
        ...
        return MonitoringStatsResponse(
            ...
            signals_processed_today=stats.get("signal_stats", {}).get(
                "total_processed", 0
            ),
            trades_executed_today=stats.get("signal_stats", {}).get(
                "auto_trades_executed", 0
            ),
            system_load={
...
async def check_authentication(
    ...
    try:
        ...
        if user and user.is_active:
            ...
            return AuthResponse(
                authenticated=False, user=None, token=None, expires_at=None
            )
    ...
    try:
        ...
        if not user or not user.is_active:
            return PermissionResponse(
                allowed=False, reason="User not found or inactive"
            )

        # Check admin privileges
        if user.is_admin:
            return PermissionResponse(allowed=True, reason="Admin user")

        # Check subscription status
        if not await service.user_manager.is_subscribed(request.telegram_id):
            return PermissionResponse(
                allowed=False, reason="Active subscription required"
            )
        ...
        if request.resource not in allowed_resources:
            return PermissionResponse(
                allowed=False, reason=f"Resource '{request.resource}' not allowed"
            )

        if request.action not in allowed_actions:
            return PermissionResponse(
                allowed=False, reason=f"Action '{request.action}' not allowed"
            )

        return PermissionResponse(allowed=True, reason="Permission granted")
