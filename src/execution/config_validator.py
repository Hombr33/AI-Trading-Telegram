...
class ConfigurationValidator:
    ...
    def _initialize_schemas(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            ...
            "paper": {
                "required": ["enabled"],
                "optional": ["initial_balance", "trading_fees", "slippage", "use_live_data"],
                "types": {
                    "enabled": bool,
                    "initial_balance": (int, float),
    ...
    def validate_platform_config(self, platform: str, config: Dict[str, Any]) -> ValidationResult:
        ...
        if not self._compatibility_manager.is_platform_available(platform):
            unavailable_platforms = self._compatibility_manager.get_unavailable_platforms()
            reason = unavailable_platforms.get(platform, "Unknown platform")
            errors.append(f"Platform '{platform}' is not available: {reason}")
            
            # Suggest alternatives
            alternatives = self._compatibility_manager.get_cross_platform_alternatives(platform)
            if alternatives:
                warnings.append(f"Consider using alternative platforms: {', '.join(alternatives)}")
        ...
        if not schema:
            if self.validation_level == ValidationLevel.STRICT:
                ...
                warnings.append(f"No validation schema found for platform '{platform}' - using basic validation")
        ...
        for field in schema.get("required", []):
            if field not in config:
                errors.append(f"Missing required field '{field}' for platform '{platform}'")
            elif config[field] is None:
                errors.append(f"Required field '{field}' cannot be None for platform '{platform}'")
        
        # Validate field types and constraints
        all_fields = schema.get("required", []) + schema.get("optional", [])
        for field in all_fields:
            if field in config:
                self._validate_field(platform, field, config[field], schema, errors, warnings)
        ...
        if unknown_fields:
            if self.validation_level == ValidationLevel.STRICT:
                errors.extend([f"Unknown field '{field}' for platform '{platform}'" for field in unknown_fields])
            else:
                warnings.extend([f"Unknown field '{field}' for platform '{platform}'" for field in unknown_fields])
        
        # Platform-specific validations
        platform_errors, platform_warnings = self._validate_platform_specific(platform, config, schema)
    ...
    def _validate_field(self, platform: str, field: str, value: Any, schema: Dict[str, Any], 
        ...
        if isinstance(value, str):
            if "min_length" in constraints and len(value) < constraints["min_length"]:
                errors.append(f"Field '{field}' for platform '{platform}' is too short (minimum {constraints['min_length']} characters)")
            
            if "max_length" in constraints and len(value) > constraints["max_length"]:
                errors.append(f"Field '{field}' for platform '{platform}' is too long (maximum {constraints['max_length']} characters)")
            
            if "pattern" in constraints:
                pattern = constraints["pattern"]
                if not re.match(pattern, value):
                    errors.append(f"Field '{field}' for platform '{platform}' doesn't match required pattern")
        
        # Numeric constraints
        if isinstance(value, (int, float)):
            if "min" in constraints and value < constraints["min"]:
                errors.append(f"Field '{field}' for platform '{platform}' is too small (minimum {constraints['min']})")
            
            if "max" in constraints and value > constraints["max"]:
                errors.append(f"Field '{field}' for platform '{platform}' is too large (maximum {constraints['max']})")
        
        # List constraints
        if isinstance(value, list):
            if "min_items" in constraints and len(value) < constraints["min_items"]:
                errors.append(f"Field '{field}' for platform '{platform}' has too few items (minimum {constraints['min_items']})")
            
            if "max_items" in constraints and len(value) > constraints["max_items"]:
                errors.append(f"Field '{field}' for platform '{platform}' has too many items (maximum {constraints['max_items']})")
    
    def _validate_platform_specific(self, platform: str, config: Dict[str, Any], schema: Dict[str, Any]) -> tuple[List[str], List[str]]:
        ...
        if platform_requirements:
            ...
            if current_os not in platform_requirements:
                errors.append(f"Platform '{platform}' requires {' or '.join(platform_requirements)} but running on {current_os}")
        ...
        if platform in ["binance", "bybit", "bitget"]:
            if config.get("enabled", False):
                ...
                if config.get("testnet", False) or config.get("sandbox", False):
                    warnings.append(f"Platform '{platform}' is configured for testnet/sandbox mode")
                
                # Validate API credentials format (basic check)
                api_key = config.get("api_key", "")
                if api_key and len(api_key) < 20:
                    warnings.append(f"API key for '{platform}' seems too short - verify it's correct")
        
        # MT5 specific validations
        if platform in ["mt5", "aiomql"]:
            if config.get("enabled", False):
                if not config.get("login") and not config.get("password"):
                    warnings.append(f"Platform '{platform}' is enabled but no login credentials provided")
                
                if config.get("server") and not config.get("server").endswith(".com"):
                    warnings.append(f"Server name for '{platform}' doesn't look like a valid MT5 server")
        
        return errors, warnings
    
    def validate_all_platforms(self, platforms_config: Dict[str, Dict[str, Any]]) -> Dict[str, ValidationResult]:
    ...
    def suggest_fixes(self, result: ValidationResult) -> List[str]:
        ...
        for error in result.errors:
            if "Missing required field" in error:
                ...
                alternatives = self._compatibility_manager.get_cross_platform_alternatives(result.platform)
                if alternatives:
                    suggestions.append(f"Use alternative platform: {alternatives[0]}")
                else:
                    suggestions.append("This platform is not supported on your operating system")
            
            elif "doesn't match required pattern" in error:
                field = error.split("'")[1]
                suggestions.append(f"Check format of field '{field}' - it may contain invalid characters")
            
            elif "too short" in error or "too long" in error:
                field = error.split("'")[1]
                suggestions.append(f"Adjust length of field '{field}' to meet requirements")
        
        return suggestions


def validate_execution_config(config: Dict[str, Any], 
                            validation_level: ValidationLevel = ValidationLevel.MODERATE) -> Dict[str, ValidationResult]:
