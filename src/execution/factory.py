...
class ExecutorFactory:
    ...
    def register_config_validator(
        self, platform: str, validator: Callable[[Dict[str, Any]], bool]
    ) -> None:
        ...
        if executor_config.auto_connect:
            try:
                connected = await asyncio.wait_for(
                    executor.connect(), timeout=executor_config.timeout
                )
                if not connected:
    ...
    def get_cache_stats(self) -> Dict[str, Any]:
        ...
        alive_refs = sum(
            1 for ref in self._executor_cache.values() if ref() is not None
        )
        return {
...
class PlatformManager(IPlatformManager):
    ...
    async def create_executor(
        ...
        if executor_config.health_check_interval > 0:
            self._start_health_monitoring(
                platform, executor_config.health_check_interval
            )
    ...
    def _start_health_monitoring(self, platform: str, interval: float) -> None:
                ...
                try:
                    ...
                    if platform in self._executors:
                        ...
                        if isinstance(health_result, dict):
                            ...
                            if status != "healthy":
                                logger.warning(
                                    f"Health check warning for {platform}: {health_result}"
                                )
        ...
        logger.debug(
            f"Started health monitoring for {platform} (interval: {interval}s)"
        )
