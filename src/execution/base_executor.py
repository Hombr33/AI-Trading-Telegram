...
class BaseExecutor(ABC):
    ...
    async def connect_with_retry(self) -> bool:
        ...
        for attempt in range(self._max_retries):
            try:
                ...
                result = await asyncio.wait_for(
                    self._connect_impl(), timeout=self._timeout
                )
                ...
                if result:
                    self.connected = True
                    self._monitor.record_connection_event(
                        self.platform_name, "connected"
                    )
                    self._monitor.record_api_call(
                        self.platform_name, "connect", response_time, True
                    )
                    logger.info(
                        f"Connected to {self.platform_name} (attempt {attempt + 1})"
                    )
                    return True
                else:
                    logger.warning(
                        f"Connection failed for {self.platform_name} (attempt {attempt + 1})"
                    )

            except asyncio.TimeoutError:
                logger.error(
                    f"Connection timeout for {self.platform_name} (attempt {attempt + 1})"
                )
            except Exception as e:
                logger.error(
                    f"Connection error for {self.platform_name}: {e} (attempt {attempt + 1})"
                )
                self._monitor.record_connection_event(
                    self.platform_name, "connection_failed"
                )
        ...
        logger.error(
            f"Failed to connect to {self.platform_name} after {self._max_retries} attempts"
        )
        return False
    ...
    async def place_order(self, request: OrderRequest) -> OrderResponse:
        ...
        try:
            ...
            # Record metrics
            self._monitor.record_order_placed(
                self.platform_name, response_time, success
            )
            self._monitor.record_api_call(
                self.platform_name, "place_order", response_time, success
            )
            ...
            self._monitor.record_api_call(
                self.platform_name, "place_order", response_time, False
            )
    ...
    def _validate_order_request(self, request: OrderRequest) -> None:
        ...
        if not request.type or request.type not in [
            "market",
            "limit",
            "stop",
            "stop_limit",
        ...
        if request.type in ["limit", "stop_limit"] and (
            not request.price or request.price <= 0
        ):
    ...
    def format_position_data(self, position: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "entry_price": float(
                position.get("entry_price", position.get("price_open", 0))
            ),
            "current_price": float(
                position.get("current_price", position.get("mark_price", 0))
            ),
            "unrealized_pnl": float(
                position.get("unrealized_pnl", position.get("profit", 0))
            ),
        ...
        try:
            ...
            if account_info and hasattr(account_info, "balance"):
                for currency, balance in account_info.balance.items():
                    self._monitor.set_account_balance(
                        self.platform_name, balance, currency
                    )
