...
class PositionManager:
        ...
        try:
            ...
            for position_data in positions:
                ...
                if ticket in self.active_positions:
                    ...
                    pos.current_price = position_data["price_open"]  # Current market price
    ...
    async def _add_position(self, position_data: Dict):
        ...
        try:
            ...
            position = Position(
                ...
                open_time=datetime.fromtimestamp(position_data["time"], tz=timezone.utc).isoformat(),
            ...
            logger.info(f"Added new position: {position_data['ticket']} {position_data['symbol']}")
    ...
    async def _check_risk_limits(self):
        ...
        try:
            ...
            if total_pnl < 0:
                ...
                if drawdown_pct >= self.config.risk_management["max_daily_drawdown_pct"]:
            ...
            if len(self.active_positions) >= self.config.risk_management["max_open_positions"]:
                logger.warning(f"Maximum open positions reached: {len(self.active_positions)}")
                await self._handle_risk_limit_breach("max_positions", len(self.active_positions))
        ...
        try:
            ...
            if breach_type == "daily_drawdown":
                ...
                if value >= self.config.risk_management["max_daily_drawdown_pct"]:
                    logger.critical("Emergency stop: Closing all positions due to excessive drawdown")
        ...
        try:
            ...
            for ticket in list(self.active_positions.keys()):
                ...
                if result["success"]:
                    ...
                    logger.error(f"Failed to emergency close position {ticket}: {result['error']}")
    ...
    async def _reduce_position_count(self):
        ...
        try:
            target_count = self.config.risk_management["max_open_positions"] - 2  # Leave room for new positions
    ...
    def get_position_stats(self) -> Dict:
        ...
        total_pnl = sum(pos.unrealized_pnl or 0 for pos in self.active_positions.values())
        
        # Count by direction
        long_positions = sum(1 for pos in self.active_positions.values() if pos.direction == "BUY")
        short_positions = sum(1 for pos in self.active_positions.values() if pos.direction == "SELL")
    ...
    def get_risk_metrics(self) -> Dict:
        ...
        return {
            ...
            "max_daily_drawdown_pct": self.config.risk_management["max_daily_drawdown_pct"]
        }
