...
class OrderManager:
    ...
    async def execute_signal(self, signal_data: Union[Signal, Dict], instrument: Optional[Instrument] = None) -> Dict:
        ...
        try:
            ...
            if instrument is None:
                instrument = type('MockInstrument', (), {
                    'id': 1,
            ...
            for setup in signal.setups:
                order = await self._create_order_from_setup(setup, signal, instrument, position_size)
                if order:
                    orders.append(order)
            ...
            for order in orders:
                ...
                if result["success"]:
                    ...
                    logger.error(f"Order placement failed: {order.order_id}: {result['error']}")
    ...
    async def modify_order(self, order_id: str, sl: Optional[float] = None, 
    ...
    async def close_position(self, position_id: str, volume: Optional[float] = None) -> Dict:
    ...
    async def _calculate_position_size(self, signal: Signal, instrument: Instrument) -> Optional[float]:
    ...
    async def _create_order_from_setup(self, setup: Dict, signal: Signal, 
                                     instrument: Instrument, position_size: float) -> Optional[Order]:
        ...
        try:
            ...
            magic_number = getattr(self.config, 'execution', {}).get('magic_number', 1001)
            ...
            order = type('MockOrder', (), {
                ...
                'stop_loss': setup["sl"],
                'take_profit': setup["tp"][0] if setup["tp"] else None,  # Use first TP level
    ...
    def get_order_stats(self) -> Dict:
        ...
        rejected_orders = sum(1 for order in self.order_history if order.status == "REJECTED")
