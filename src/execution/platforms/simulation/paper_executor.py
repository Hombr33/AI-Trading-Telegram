...
class PaperExecutor(BaseExecutor):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self._data_source = config.get(
            "data_source", "paper"
        )  # Can integrate with real exchanges
        self._use_live_data = config.get(
            "use_live_data", True
        )  # Default to True for auto-trading
        ...
        self._partial_fill_probability = config.get(
            "partial_fill_probability", 0.05
        )  # Reduced for auto-trading
        self._rejection_probability = config.get(
    ...
    async def _place_order_impl(self, request: OrderRequest) -> OrderResponse:
        ...
        if random.random() < self._rejection_probability:
            raise RuntimeError(
                "Order rejected: Insufficient margin or invalid parameters"
            )
        ...
        execution_price = self._calculate_execution_price(request, current_price)

        # Simulate partial fills
        filled_amount = request.amount
        if (
            random.random() < self._partial_fill_probability
            and request.type != "market"
        ):
        ...
        logger.info(
            f"Paper order placed: {order_id} for {request.symbol} at {execution_price}"
        )
        return response
    ...
    async def _get_market_price(self, symbol: str) -> float:
        ...
        if (
            symbol in self._market_data_cache
            and symbol in self._last_price_update
            and current_time - self._last_price_update[symbol] < 60
        ):  # 1 minute cache
        ...
        if "USDT" in symbol_upper:  # Crypto
            ...
            forex in symbol_upper
            for forex in ["EUR", "GBP", "USD", "AUD", "CAD", "CHF", "NZD", "JPY"]
        ):  # Forex
        ...
        if position_id in self._positions:
            ...
            if order.side == position.side:
                ...
                if position.size > order.filled:
                    ...
                    position.margin = (position.margin or 0) - (
                        order.price * order.filled
                    )
                    # Realize some PnL
    ...
    async def get_ticker(self, symbol: str) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            "metadata": {
                ...
                "simulated": not self._use_live_data,
            },
        }
