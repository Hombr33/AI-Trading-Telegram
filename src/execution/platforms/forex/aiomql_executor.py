...
from ....core.error_handler import with_error_handling, ErrorContext, CircuitBreaker
from ....core.exceptions import MT5ConnectionError, MT5ExecutionError, AioMQLError
...
class AioMQLExecutor(MT5Executor):
    ...
    def __init__(self, config):
        ...
        self._connection_circuit_breaker = CircuitBreaker(
            failure_threshold=3, recovery_timeout=60.0, expected_exception=AioMQLError
        )
            ...
            async with ErrorContext(
                ...
                try:
                    ...
                    if not self._connection_circuit_breaker.can_execute():
                        logger.warning(
                            "Connection circuit breaker is open, using fallback"
                        )
                            ...
                            try:
                                ...
                                if self.account_info:
                                    log_system_event(
                                        ...
                                        context={
                                            "account_balance": self.account_info.get(
                                                "balance", 0
                                            )
                                        },
                                    )
                                ...
                                logger.warning(
                                    f"Failed to get aiomql account info: {e}"
                                )

                            log_operation_timing(
                                "aiomql_connect", start_time, time.time()
                            )
                            return True
                        else:
    ...
    async def disconnect(self):
        ...
        if (
            _AIOMQL_AVAILABLE
            and self.connected
            and (self._ai_session or self._ai_client)
        ):
            try:
                if self._ai_session and hasattr(self._ai_session, "shutdown"):
    ...
    async def place_order(self, order) -> Dict:
        ...
        if _AIOMQL_AVAILABLE and self.connected and self._ai_session:
            try:
                ...
                comment = str(order.get("comment", "AioMQL Order"))[
                    :31
                ]  # MT5 comment limit
                ...
                try:
                    ...
                    if result and hasattr(result, "retcode"):
                        if result.retcode == 10009:  # TRADE_RETCODE_DONE
                            ...
                            logger.info(
                                f"Order placed successfully via aiomql: {order_id}"
                            )
                            return {
                                "order_id": order_id,
                                "symbol": symbol,
                            ...
                            logger.warning(
                                f"aiomql order failed with retcode: {result.retcode}"
                            )
                    else:
                        logger.warning(
                            f"aiomql order placement returned invalid result: {result}"
                        )
            ...
            try:
                ...
                if order:
                    ...
                    if (
                        ...
                        logger.info(
                            f"Order {order_id} modified successfully via aiomql"
                        )
                        return {
                            "order_id": order_id,
                            "sl": sl if sl is not None else order.get("sl", 0.0),
                        ...
                        logger.warning(
                            f"aiomql order modification returned invalid result: {result}"
                        )
                else:
        ...
        if _AIOMQL_AVAILABLE and self.connected and self._ai_session:
            try:
                ...
                if position:
                    ...
                    close_volume = (
                        volume if volume is not None else position.get("volume", 0.01)
                    )
                    ...
                    if (
                        ...
                        logger.info(
                            f"Position {position_id} closed successfully via aiomql"
                        )
                        return {
                        ...
                        logger.warning(
                            f"aiomql position close returned invalid result: {result}"
                        )
                else:
                    ...
                    for pos in positions:
                        try:
                            ...
                            if ticket is None or symbol is None or pos_type is None:
                                logger.warning(
                                    f"Skipping position with missing data: {pos}"
                                )
                                continue

                            position_dict = {
                                "position_id": (
                                    int(ticket) if isinstance(ticket, (int, str)) else 0
                                ),
                            ...
                            logger.warning(
                                f"Error processing position {getattr(pos, 'ticket', 'unknown')}: {e}"
                            )
                            continue
                        ...
                        result.append(
                            {
                                ...
                                "type": self._map_order_type_reverse(
                ...
                if hasattr(self._ai_client, "info"):
                    ...
                    if account_info is not None:
                        ...
                        return {
                            ...
                            "server": getattr(
                                account_info, "server", self.config.server
                            ),
    ...
    async def get_symbol_info(self, symbol: str) -> Optional[Dict[str, Any]]:
        ...
        # Fallback to standard MT5Executor
        return (
            await super().get_symbol_info(symbol)
            if hasattr(super(), "get_symbol_info")
            else None
        )
