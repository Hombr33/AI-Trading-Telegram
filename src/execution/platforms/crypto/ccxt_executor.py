...
class CCXTExecutor(BaseExecutor):
    ...
    async def _place_order_impl(self, request) -> 'OrderResponse':
        ...
        try:
            ...
            if order_type == 'market':
                ...
                result = await self.exchange.create_order(symbol, order_type, side, amount, price)
            
            # Log trade event
            log_trade_event(self.exchange_name, "order_placed", {
                "symbol": symbol,
                "side": side,
                "type": order_type,
                "amount": amount,
            ...
            return OrderResponse(
                ...
                timestamp=datetime.fromtimestamp(result.get("timestamp", 0) / 1000, timezone.utc) if result.get("timestamp") else datetime.now(timezone.utc),
    ...
    async def _place_order_ccxt(self, order: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            if not self.exchange:
                return {"success": False, "error": f"Not connected to {self.exchange_name}"}
            ...
            # Place order using CCXT
            if order_type == 'market':
                result = await self.exchange.create_market_order(symbol, side, amount, None, None, params)
            elif order_type == 'limit':
                if price is None:
                    return {"success": False, "error": "Price required for limit orders"}
                result = await self.exchange.create_limit_order(symbol, side, amount, price, None, params)
            else:
                result = await self.exchange.create_order(symbol, order_type, side, amount, price, None, params)
    ...
    async def cancel_order(self, order_id: str, symbol: str = None) -> Dict[str, Any]:
        ...
        try:
            ...
            log_trade_event(self.exchange_name, "order_cancelled", {
                "symbol": symbol,
                "order_id": order_id
            })
