...
class HealthMonitor:
    ...
    def register_component(self, 
                          ...
                          initial_status: HealthStatus = HealthStatus.UNKNOWN) -> None:
    ...
    def add_status_callback(self, component: str, callback: Callable[[HealthCheckResult], None]) -> None:
        """Add callback for health status changes."""
        self._callbacks[component].append(callback)
    
    @with_error_handling("perform_health_check", fallback_value=None)
    async def perform_health_check(self, component: str, checker: IHealthCheckable) -> Optional[HealthCheckResult]:
    ...
    def get_recent_results(self, component: Optional[str] = None, count: int = 100) -> List[HealthCheckResult]:
    ...
    def get_component_history(self, component: str, hours: int = 24) -> List[HealthCheckResult]:
...
class ExecutorMonitor:
    ...
    def record_order_placed(self, platform: str, response_time_ms: float, success: bool) -> None:
    ...
    def record_api_call(self, platform: str, endpoint: str, response_time_ms: float, success: bool) -> None:
        """Record API call metrics."""
        self._performance_monitor.increment_counter(
            "api_calls_total",
            labels={"platform": platform, "endpoint": endpoint, "status": "success" if success else "failed"}
        )
        self._performance_monitor.record_timer(
            "api_call_duration",
            response_time_ms,
    ...
    def set_account_balance(self, platform: str, balance: float, currency: str = "USD") -> None:
    ...
    def get_overall_status(self) -> Dict[str, Any]:
        ...
        healthy_count = sum(1 for status in statuses.values() if status == HealthStatus.HEALTHY)
        total_count = len(statuses)
        
        overall_health = "healthy" if healthy_count == total_count and total_count > 0 else "degraded"
