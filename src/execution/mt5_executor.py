...
try:
    ...
    class MockMT5:
        ...
        def history_orders_get(self, group='last', offset=0, count=10000, extended=False):
...
class MT5Executor:
    ...
    async def modify_order(self, order_id: int, sl: Optional[float] = None, 
            ...
            return [
                {
                    ...
                    "price_open": pos.price_open if hasattr(pos, 'price_open') else 1.2000,
                    "price_current": pos.price_current if hasattr(pos, 'price_current') else 1.2000,
                    ...
                    "time": pos.time if hasattr(pos, 'time') else int(datetime.now().timestamp())
                }
            ...
            return [
                {
                    ...
                    "time": order.time if hasattr(order, 'time') else int(datetime.now().timestamp())
                }
    ...
    def _prepare_order_request(self, order: Order) -> Dict:
        ...
        return {
            ...
            "deviation": self.config.execution["slippage_points"],
            "magic": self.config.execution["magic_number"],
            "comment": order.comment or f"AI_SIGNAL_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
            "type_time": mt5.ORDER_TIME_GTC,
