...
class TrailingManager:
        ...
        try:
            ...
            for position in positions:
                ...
                if ticket in self.active_positions:
                    ...
                    pos_state.current_price = position["price_open"]  # Use current market price
    ...
    async def _process_position_trailing(self, pos_state: PositionState):
        ...
        if pos_state.direction == "BUY":
            profit_points = (pos_state.current_price - pos_state.open_price) * 10000  # Convert to points
        else:
        ...
        if not pos_state.trailing_activated:
            if (pos_state.partial_tp_hit and profit_points >= 200) or profit_points >= 200:
    ...
    async def _move_to_breakeven(self, pos_state: PositionState):
        ...
        try:
            ...
            if result["success"]:
                ...
                logger.error(f"Failed to move position {pos_state.ticket} to breakeven: {result['error']}")
    ...
    async def _take_partial_profit(self, pos_state: PositionState):
        ...
        try:
            ...
            if result["success"]:
                ...
                logger.error(f"Failed to take partial profit for position {pos_state.ticket}: {result['error']}")
                
        except Exception as e:
            logger.error(f"Error taking partial profit for position {pos_state.ticket}: {e}")
    ...
    async def _activate_trailing(self, pos_state: PositionState):
        ...
        try:
            ...
            if pos_state.direction == "BUY":
                trailing_stop = pos_state.current_price - (self.config.stop_points / 10000)
            else:
                trailing_stop = pos_state.current_price + (self.config.stop_points / 10000)
            ...
            if result["success"]:
                ...
                logger.info(f"Trailing activated for position {pos_state.ticket} at {trailing_stop}")
            else:
                logger.error(f"Failed to activate trailing for position {pos_state.ticket}: {result['error']}")
                
        except Exception as e:
            logger.error(f"Error activating trailing for position {pos_state.ticket}: {e}")
    ...
    async def _update_trailing_stop(self, pos_state: PositionState):
        ...
        try:
            if pos_state.direction == "BUY":
                ...
                if new_stop > pos_state.stop_loss + (self.config.step_points / 10000):
                    ...
                    if result["success"]:
                        ...
                        logger.info(f"Trailing stop updated for position {pos_state.ticket} to {new_stop}")
            ...
            logger.error(f"Error updating trailing stop for position {pos_state.ticket}: {e}")
    ...
    def get_trailing_stats(self) -> Dict:
        ...
        trailing_active = sum(1 for pos in self.active_positions.values() if pos.trailing_activated)
        breakeven_moved = sum(1 for pos in self.active_positions.values() if pos.breakeven_moved)
        partial_tp_hit = sum(1 for pos in self.active_positions.values() if pos.partial_tp_hit)
