...
class MultiUserService:
    ...
    def __init__(self, telegram_bot_token: str):
        ...
        self.order_manager = MultiUserOrderManager(
            self.ea_bridge, self.position_manager, self.user_manager, self.config_manager
        )
    ...
    async def process_signal(self, signal_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            execution_results = await self._execute_signal_trades(signal_data, distribution_plan["immediate"])
            ...
            return {
                "success": True,
                "distributed_to": distribution_plan["immediate"] + distribution_plan["delayed"] + distribution_plan["batch"],
    ...
    async def _enrich_signal_data(self, signal_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            signal_data["signal_id"] = signal_data.get("signal_id", f"sig_{int(datetime.now(timezone.utc).timestamp())}")
            ...
            if setups:
                ...
                if entry_zone and sl and tp:
                    ...
                    entry_price = sum(entry_zone) / len(entry_zone) if len(entry_zone) == 2 else entry_zone[0]
                    ...
                    signal_data["risk_metrics"] = {
                        ...
                        "recommended_position_size": self._calculate_position_size(risk, signal_data)
                    }
    ...
    async def _create_distribution_plan(self, signal_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            subscribers = await self.signal_distributor.get_subscribed_users(symbol, confidence)
            ...
            for subscriber in subscribers:
                ...
                user_config = await self.config_manager.get_all_user_configs(telegram_id)
                should_distribute = await self.signal_distributor.should_distribute_signal(signal_data, user_config)
                ...
                preferences = await self.signal_distributor.get_user_signal_preferences(telegram_id)
    ...
    async def _queue_signals_for_distribution(self, signal_data: Dict[str, Any], distribution_plan: Dict[str, Any]) -> None:
        ...
        try:
            ...
            for telegram_id in distribution_plan["delayed"]:
                ...
                delayed_time = datetime.now(timezone.utc) + timedelta(minutes=delay_minutes)
            ...
            logger.info(f"Queued signal for {len(distribution_plan['immediate'])} immediate, "
    ...
    def _calculate_position_size(self, risk_amount: float, signal_data: Dict[str, Any]) -> float:
    ...
    async def _distribute_signal_to_user(self, signal_data: Dict[str, Any], telegram_id: int, distribution_type: str) -> None:
        """Distribute signal to a specific user."""
        try:
            # Format signal message
            message = await self.signal_distributor.format_signal_message(signal_data, telegram_id)
            ...
            if self.notification_manager:
                ...
                await self.telegram_bot.send_message(telegram_id, message, parse_mode="Markdown")
            else:
                logger.error("No notification system available")

            logger.info(f"Signal distributed to user {telegram_id} ({distribution_type})")

        except Exception as e:
            logger.error(f"Error distributing signal to user {telegram_id}: {e}")
            raise

    async def _send_batch_notification(self, telegram_id: int, signals: List[Dict[str, Any]]) -> None:
        ...
        try:
            ...
            logger.info(f"Batch notification sent to user {telegram_id} with {len(signals)} signals")
            ...
            if self._batch_queue:
                ...
                for telegram_id, signals in user_signals.items():
                    try:
                        ...
                        logger.error(f"Error flushing batch signals for {telegram_id}: {e}")
    ...
    def _update_signal_stats(self, distribution_plan: Dict[str, Any]) -> None:
        ...
        try:
            ...
            self._signal_stats["total_distributed"] += len(distribution_plan["immediate"]) + len(distribution_plan["delayed"]) + len(distribution_plan["batch"])
            self._signal_stats["total_skipped"] += len(distribution_plan["skipped"])
            self._signal_stats["last_signal_time"] = datetime.now(timezone.utc).isoformat()
    ...
    async def _monitor_position(self, telegram_id: int, position: Dict[str, Any], platform: str) -> None:
        ...
        try:
            ...
            risk_config = await self.config_manager.get_user_config(telegram_id, "risk_management")
            if not risk_config:
                risk_config = {}
            ...
            await self._update_trailing_stops(telegram_id, position, platform, risk_config)
            
            # Check for margin/drawdown alerts
            await self._check_risk_alerts(telegram_id, position, risk_config)
            
            # Log position status
            logger.debug(f"Monitored position {ticket} for user {telegram_id}: "
                        f"{symbol} {position_type} {volume} @ {current_price} P&L: {profit}")

        except Exception as e:
            logger.error(f"Error monitoring position for user {telegram_id}: {e}")

    async def _check_profit_loss_alerts(self, telegram_id: int, position: Dict[str, Any], risk_config: Dict[str, Any]) -> None:
        ...
        try:
            ...
            profit_alert_threshold = risk_config.get('profit_alert_threshold', 50.0)  # $50 profit
            loss_alert_threshold = risk_config.get('loss_alert_threshold', -25.0)  # $25 loss
    ...
    async def _check_stop_loss_hit(self, telegram_id: int, position: Dict[str, Any], platform: str) -> None:
    ...
    async def _check_take_profit_hit(self, telegram_id: int, position: Dict[str, Any], platform: str) -> None:
    ...
    async def _update_trailing_stops(self, telegram_id: int, position: Dict[str, Any], platform: str, risk_config: Dict[str, Any]) -> None:
        """Update trailing stops if enabled."""
        try:
            ...
            trailing_distance = risk_config.get('trailing_stop_distance', 20)  # 20 pips default
            trailing_type = risk_config.get('trailing_stop_type', 'pips')  # 'pips' or 'percentage'
            ...
            if position_type in ['BUY', 'Buy', 0]:
                ...
                if trailing_type == 'pips':
                    potential_sl = current_price - (trailing_distance * 0.0001)  # Assuming 4-digit quotes
                else:  # percentage
            ...
            if new_sl and abs(new_sl - (current_sl or 0)) > 0.00001:  # Avoid tiny updates
                try:
                    if platform == "mt5":
                        await self.ea_bridge.modify_position(telegram_id, ticket, sl=new_sl)
                    elif platform == "crypto":
                        await self.crypto_bridge.modify_position(telegram_id, ticket, stop_loss=new_sl)
    ...
    async def _check_risk_alerts(self, telegram_id: int, position: Dict[str, Any], risk_config: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if hasattr(self, 'ea_bridge'):
                try:
                    account_info = await self.ea_bridge.get_account_info(telegram_id) or {}
                except Exception:
                    pass
            
            # Check drawdown alerts
            max_drawdown_pct = risk_config.get('max_drawdown_percent', 10.0)  # 10% max drawdown
            ...
            if balance > 0 and equity > 0:
                ...
                if current_drawdown >= max_drawdown_pct:
                    ...
                    message += f"🛑 Consider reducing position sizes or stopping trading"
                    
                    await self._send_position_alert(telegram_id, message)
            
            # Check margin level alerts
            margin_level = account_info.get('margin_level', 0)
            min_margin_level = risk_config.get('min_margin_level', 200.0)  # 200% minimum
    ...
    async def _send_position_alert(self, telegram_id: int, message: str) -> None:
        ...
        try:
            if self.notification_manager:
                ...
                logger.warning(f"Cannot send position alert to {telegram_id}: No notification system available")
    ...
    async def _perform_health_check(self) -> None:
        ...
        health_status = {
            ...
            "crypto_bridge": {"status": "unknown", "error": None, "connected_exchanges": 0},
        ...
        try:
            ...
            if overall_status == "healthy":
                ...
                logger.warning(f"Health check completed with warnings: {self._get_health_issues(health_status)}")
            else:
                logger.error(f"Health check completed with critical issues: {self._get_health_issues(health_status)}")
            ...
            if self.telegram_bot:
                try:
                    await self.telegram_bot.send_admin_alert(f"Health check system failure: {e}")
    ...
    async def _check_database_health(self, health_status: Dict[str, Any]) -> None:
        ...
        try:
            ...
            health_status["database"]["status"] = "critical"
            health_status["database"]["error"] = f"Database import/initialization failed: {e}"
            ...
            if self.telegram_bot:
                ...
                try:
                    ...
                    if bot_info:
                        ...
                        health_status["telegram_bot"]["bot_username"] = bot_info.username
                        logger.debug(f"Telegram bot health check: HEALTHY - {bot_info.username}")
                    else:
                ...
                health_status["telegram_bot"]["status"] = "critical"
                health_status["telegram_bot"]["error"] = "Telegram bot instance not available"
    ...
    async def _check_ea_bridge_health(self, health_status: Dict[str, Any]) -> None:
        ...
        try:
            if self.ea_bridge:
                ...
                try:
                    connection_health = await self.ea_bridge.get_all_user_connections_health()
                    
                    if connection_health:
                        connected_users = len([conn for conn in connection_health.values() if conn.get('connected')])
                        ...
                        if connected_users == total_users and total_users > 0:
                            ...
                            health_status["ea_bridge"]["error"] = f"Only {connected_users}/{total_users} EA connections active"
                        else:
                            health_status["ea_bridge"]["status"] = "warning"
                            health_status["ea_bridge"]["error"] = "No active EA connections"
                            
                        logger.debug(f"EA bridge health check: {connected_users}/{total_users} connections active")
                    else:
                        health_status["ea_bridge"]["status"] = "warning"
                        health_status["ea_bridge"]["error"] = "No EA connection data available"
        ...
        try:
            if self.crypto_bridge:
                ...
                try:
                    ...
                    health_status["crypto_bridge"]["connected_exchanges"] = 1  # Placeholder
                ...
                health_status["crypto_bridge"]["status"] = "warning"
                health_status["crypto_bridge"]["error"] = "Crypto bridge not initialized"
    ...
    async def _check_signal_queue_health(self, health_status: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if total_queued > max_queue_size:
                ...
                health_status["signal_queues"]["error"] = f"High queue size: {total_queued} items"
                logger.warning(f"Signal queue health: WARNING - {total_queued} items queued")
            else:
                health_status["signal_queues"]["status"] = "healthy"
                logger.debug(f"Signal queue health: HEALTHY - {total_queued} items queued")
    ...
    async def _check_memory_usage(self, health_status: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if memory_usage_mb > critical_threshold:
                ...
                health_status["memory_usage"]["error"] = f"High memory usage: {memory_usage_mb:.2f} MB"
                logger.error(f"Memory usage health: CRITICAL - {memory_usage_mb:.2f} MB")
            elif memory_usage_mb > warning_threshold:
                health_status["memory_usage"]["status"] = "warning"
                health_status["memory_usage"]["error"] = f"Elevated memory usage: {memory_usage_mb:.2f} MB"
                logger.warning(f"Memory usage health: WARNING - {memory_usage_mb:.2f} MB")
            else:
    ...
    def _calculate_overall_health_status(self, health_status: Dict[str, Any]) -> None:
        ...
        try:
            ...
            # Count status types
            for component in ["database", "telegram_bot", "ea_bridge", "crypto_bridge", "signal_queues", "memory_usage"]:
            ...
            if critical_count > 0:
                ...
                if db_status == "critical" or bot_status == "critical":
                    ...
                    health_status["overall_status"] = "warning"  # Other components critical but core is ok
    ...
    async def initialize_user_trading_session(self, telegram_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            position_tracking = await self.position_manager.initialize_user_tracking(telegram_id)

            # Get user risk metrics
            risk_metrics = await self.position_manager.get_user_risk_metrics(telegram_id)
    ...
    async def submit_user_order(self, telegram_id: int, order_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if result["success"]:
                logger.info(f"Order submitted for user {telegram_id}: {result.get('order_id')}")
            else:
                logger.warning(f"Order submission failed for user {telegram_id}: {result.get('error')}")
    ...
    async def get_user_trading_status(self, telegram_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            # Get pending orders
            pending_orders = await self.order_manager.get_user_pending_orders(telegram_id)

            # Get recent order history
            order_history = await self.order_manager.get_user_order_history(telegram_id, limit=10)
            ...
            return {
                ...
                "recent_orders": [self._order_to_dict(order) for order in order_history],
    ...
    def _position_to_dict(self, position) -> Dict[str, Any]:
        ...
        return {
            ...
            "symbol": getattr(position, 'instrument', {}).symbol if hasattr(position, 'instrument') else "Unknown",
    ...
    async def modify_user_position(self, telegram_id: int, ticket: int,
        ...
        try:
            result = await self.position_manager.modify_user_position(telegram_id, ticket, sl, tp)

            if result["success"]:
                logger.info(f"Position {ticket} modified for user {telegram_id}")
            else:
                logger.warning(f"Position modification failed for user {telegram_id}: {result.get('error')}")
    ...
    async def close_user_position(self, telegram_id: int, ticket: int, volume: Optional[float] = None) -> Dict[str, Any]:
        ...
        try:
            result = await self.position_manager.close_user_position(telegram_id, ticket, volume)

            if result["success"]:
                logger.info(f"Position {ticket} closed for user {telegram_id}")
            else:
                logger.warning(f"Position close failed for user {telegram_id}: {result.get('error')}")
    ...
    async def cancel_user_order(self, telegram_id: int, order_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if result["success"]:
                ...
                logger.warning(f"Order cancellation failed for user {telegram_id}: {result.get('error')}")
    ...
    async def emergency_user_stop(self, telegram_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            cancel_result = await self.order_manager.emergency_cancel_all_user_orders(telegram_id)

            # Close all positions
            close_result = await self.position_manager.emergency_close_all_user_positions(telegram_id)
    ...
    async def get_all_users_trading_status(self, admin_telegram_id: int) -> Dict[str, Any]:
