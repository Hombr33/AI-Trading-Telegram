...
class AutoTradingService(IAutoTradingService):
        ...
        while self.running:
            try:
                ...
                if self.trades_today >= config.auto_trading.max_trades_per_day:
                    logger.info(f"Daily trade limit reached: {self.trades_today}/{config.auto_trading.max_trades_per_day}")
    ...
    async def _execute_signal(self, signal: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if result.get("success"):
                ...
                logger.info(f"Auto trade executed: {action} {position_size} {symbol} @ {signal.get('entry_price')}")
                return True
            else:
                logger.error(f"Failed to execute auto trade for {symbol}: {result.get('error')}")
                return False
    ...
    async def _calculate_position_size(self, signal: Dict[str, Any]) -> Optional[float]:
        ...
        try:
            ...
            logger.info(f"Calculated position size for {symbol}: {position_size} (risk: {config.auto_trading.risk_per_trade_percent}%)")
    ...
    async def _is_trade_open(self, trade_info: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            for position in positions:
                if (position.get("order_id") == order_id or 
    ...
    async def _handle_trade_closure(self, symbol: str, trade_info: Dict[str, Any]):
        ...
        try:
            ...
            log_trade_event(
                ...
                {
                    "platform": trade_info.get("platform"),
                    "auto_trade": True
                }
            )
    ...
    async def _send_trade_notification(self, trade_info: Dict[str, Any], event_type: str):
