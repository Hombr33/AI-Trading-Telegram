...
from ..models.telegram_users import (
    TelegramUser, UserRole, SubscriptionStatus, UserConfiguration,
    PlatformConnection, PlatformType, SignalSubscription, ServerConfiguration
)
...
class UserManager:
    ...
    async def create_user(self, telegram_id: int, username: str = None, first_name: str = None, last_name: str = None) -> bool:
        ...
        try:
            with SessionLocal() as session:
                ...
                if not user:
                    # Create new user
                    role = UserRole.ADMIN if telegram_id == self.initial_admin_id else UserRole.USER
                    user = TelegramUser(
                        ...
                        last_name=last_name,
                        role=role,
                        subscription_status=SubscriptionStatus.ACTIVE if role == UserRole.ADMIN else SubscriptionStatus.EXPIRED
                    )
                    session.add(user)
                    session.commit()
                    session.refresh(user)
                    logger.info(f"Created new user: {telegram_id} with role {role.value}")
                    return True
                else:
    ...
    async def unsubscribe_from_symbol(self, telegram_id: int, symbol: str) -> bool:
        ...
        try:
            with SessionLocal() as session:
                ...
                subscription = session.query(SignalSubscription).filter(
                ...
                if subscription:
                    ...
                    session.commit()
                    logger.info(f"User {telegram_id} unsubscribed from {symbol} signals")
                    return True
                else:
                    return False
        except Exception as e:
    ...
    async def add_admin(self, admin_telegram_id: int, target_telegram_id: int) -> bool:
        ...
        try:
            ...
            with SessionLocal() as session:
                user = session.query(TelegramUser).filter(
                    TelegramUser.telegram_id == target_telegram_id
                ).first()
                ...
                if user:
                    ...
                    session.commit()
                    logger.info(f"User {target_telegram_id} promoted to admin by {admin_telegram_id}")
                    return True
                else:
                    return False
        except Exception as e:
    ...
    async def remove_admin(self, admin_telegram_id: int, target_telegram_id: int) -> bool:
        ...
        try:
            ...
            with SessionLocal() as session:
                ...
                if user and user.is_admin:
                    ...
                    session.commit()
                    logger.info(f"Admin privileges removed from {target_telegram_id} by {admin_telegram_id}")
                    return True
                else:
                    return False
        except Exception as e:
    ...
    async def set_subscription(self, admin_telegram_id: int, target_telegram_id: int, status: SubscriptionStatus, expires_at: datetime = None) -> bool:
        ...
        try:
            ...
            with SessionLocal() as session:
                ...
                if user:
                    ...
                    session.commit()
                    logger.info(f"Subscription for {target_telegram_id} set to {status.value} by {admin_telegram_id}")
                    return True
                else:
                    return False
        except Exception as e:
    ...
    async def get_all_users(self, admin_telegram_id: int) -> Optional[List[Dict[str, Any]]]:
    ...
    async def register_platform_connection(self, telegram_id: int, platform_type: PlatformType, connection_name: str, api_key: str, api_secret: str = None, server_endpoint: str = None) -> bool:
        ...
        try:
            ...
            with SessionLocal() as session:
                ...
                session.commit()
                logger.info(f"Platform connection registered for user {telegram_id}: {platform_type.value}")
                return True
        except Exception as e:
            logger.error(f"Failed to register platform connection: {e}")
            return False

    async def get_user_platform_connections(self, telegram_id: int) -> List[Dict[str, Any]]:
        ...
        try:
            with SessionLocal() as session:
                ...
                connections = session.query(PlatformConnection).filter(
                    PlatformConnection.user_id == user.id,
                ...
                return [
                    {
                        ...
                        "api_key": conn.api_key[:8] + "..." if conn.api_key else None,  # Masked
    ...
    async def subscribe_to_symbol(self, telegram_id: int, symbol: str, min_confidence: int = 60) -> bool:
        ...
        try:
            ...
            with SessionLocal() as session:
                ...
                existing = session.query(SignalSubscription).filter(
                    SignalSubscription.user_id == user.id,
                    SignalSubscription.symbol == symbol
                ).first()
    ...
    async def get_user_subscriptions(self, telegram_id: int) -> List[Dict[str, Any]]:
        ...
        try:
            with SessionLocal() as session:
                ...
                subscriptions = session.query(SignalSubscription).filter(
                    SignalSubscription.user_id == user.id,
