...
class UserConfigService:
    ...
    async def get_user_config(self, telegram_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                configs = session.query(UserConfiguration).filter(
    ...
    async def update_user_config(self, telegram_id: int, config_section: str, 
        ...
        try:
            ...
            try:
                ...
                logger.info(f"Updated config for user {telegram_id}: {config_section}.{key} = {value}")
                return True
            finally:
                session.close()
    ...
    async def toggle_notification(self, telegram_id: int, notification_type: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Error toggling notification {notification_type} for {telegram_id}: {e}")
            return False
    ...
    async def get_user_symbols(self, telegram_id: int) -> List[str]:
        ...
        try:
            config = await self.get_user_config(telegram_id)
            return config.get("trading", {}).get("allowed_symbols", 
                                                 self.default_config["trading"]["allowed_symbols"])
        except Exception as e:
    ...
    async def validate_config_update(self, config_section: str, key: str, value: Any) -> tuple[bool, str]:
        ...
        try:
            if config_section == "risk":
                if key in ["max_drawdown_pct", "max_daily_loss_pct", "max_position_size_pct"]:
                ...
                if key == "update_frequency_seconds":
                    if not isinstance(value, int) or value < 10 or value > 3600:
                        return False, "Update frequency must be between 10 and 3600 seconds"
