...
class ConfigManager:
    ...
    async def set_user_config(self, telegram_id: int, config_type: str,
                              config_data: Dict[str, Any], validate: bool = True) -> bool:
        ...
        try:
            with get_db_session() as session:
                ...
                if validate:
                    is_valid, error_msg = await self.validate_config_comprehensive(config_type, config_data)
                    if not is_valid:
                        logger.warning(f"Configuration validation failed for user {telegram_id}: {error_msg}")
                        return False

                # Check for configuration dependencies
                dependency_errors = await self._check_config_dependencies(config_type, config_data, telegram_id)
                if dependency_errors:
                    logger.warning(f"Configuration dependency issues for user {telegram_id}: {dependency_errors}")
    ...
    async def validate_config_comprehensive(self, config_type: str, config_data: Dict[str, Any]) -> Tuple[bool, str]:
    ...
    def _validate_risk_config(self, config_data: Dict[str, Any]) -> List[str]:
        ...
        if not isinstance(drawdown_pct, (int, float)) or drawdown_pct <= 0 or drawdown_pct > 20:
        ...
        if not isinstance(daily_loss, (int, float)) or daily_loss <= 0 or daily_loss > 1000:
        ...
        if not isinstance(target_profit, (int, float)) or target_profit <= 0 or target_profit > 2000:
        ...
        if not isinstance(loss_rules, list) or len(loss_rules) == 0:
            ...
            for i, rule in enumerate(loss_rules):
                ...
                if "losses" not in rule or "action" not in rule:
                    errors.append(f"Loss rule {i+1} must have 'losses' and 'action' fields")
    ...
    def _validate_symbol_config(self, config_data: Dict[str, Any]) -> List[str]:
        ...
        if not isinstance(active_symbols, list) or len(active_symbols) == 0:
            ...
            valid_symbols = ["XAUUSD", "EURUSD", "GBPUSD", "USDJPY", "USDCAD", "AUDUSD", "USDCHF", "NZDUSD"]
        ...
        if not isinstance(symbol_settings, dict):
            ...
            for symbol, settings in symbol_settings.items():
                ...
                if not isinstance(min_conf, int) or min_conf < 30 or min_conf > 95:
                    errors.append(f"Min confidence for {symbol} must be between 30 and 95")

                # Spread validation
                max_spread = settings.get("max_spread")
                if not isinstance(max_spread, (int, float)) or max_spread < 0 or max_spread > 20:
    ...
    async def bulk_set_user_configs(self, telegram_id: int, configs: Dict[str, Dict[str, Any]]) -> Dict[str, bool]:
        ...
        for config_type, config_data in configs.items():
            ...
            is_valid, error_msg = await self.validate_config_comprehensive(config_type, config_data)
            if is_valid:
                results[config_type] = await self.set_user_config(telegram_id, config_type, config_data)
            else:
    ...
    async def get_config_history(self, telegram_id: int, config_type: str = None,
        ...
        try:
            with get_db_session() as session:
                ...
                configs = query.order_by(desc(UserConfiguration.updated_at)).limit(limit).all()
    ...
    async def clone_user_config(self, source_telegram_id: int, target_telegram_id: int,
                              config_types: List[str] = None) -> bool:
        ...
        try:
            ...
            results = await self.bulk_set_user_configs(target_telegram_id, filtered_configs)
            return all(results.values())
    ...
    async def get_users_with_config(self, config_type: str, config_key: str = None,
                                  config_value: Any = None) -> List[int]:
        """Get telegram IDs of users with specific configuration."""
        try:
            with get_db_session() as session:
                query = session.query(TelegramUser.telegram_id).join(UserConfiguration).filter(
                    UserConfiguration.config_type == config_type,
                    UserConfiguration.is_active == True
                )
                ...
                if config_key and config_value is not None:
                    ...
                    query = query.filter(UserConfiguration.config_data.contains({config_key: config_value}))
    ...
    def _get_cached_config(self, telegram_id: int, config_type: str) -> Optional[Dict[str, Any]]:
    ...
    async def get_config_statistics(self, admin_telegram_id: int) -> Optional[Dict[str, Any]]:
            ...
            with get_db_session() as session:
                ...
                for config_type, is_active, count in config_counts:
                    ...
                    if is_active:
                        ...
                        stats["configurations_by_type"][config_type]["inactive"] += count
    ...
    async def _check_config_dependencies(self, config_type: str, config_data: Dict[str, Any],
                                       telegram_id: int) -> List[str]:
        ...
        try:
            if config_type == "symbol":
                ...
                if signal_config:
                    active_symbols = config_data.get("active_symbols", [])
                    min_conf_threshold = signal_config.get("min_confidence_threshold", 60)

                    symbol_settings = config_data.get("symbol_settings", {})
                    for symbol in active_symbols:
                        symbol_min_conf = symbol_settings.get(symbol, {}).get("min_confidence", 60)
                ...
                if risk_config:
                    ...
                    if risk_pct > 3.0 and min_rr < 1.5:
                        warnings.append("High risk per trade with low risk-reward ratio may be dangerous")
                ...
                if trading_config:
                    ...
                    sizing_method = trading_config.get("position_sizing_method", "risk_percent_of_equity")

                    if sizing_method == "fixed_lot" and max_positions > 20:
                        warnings.append("Fixed lot sizing with many open positions may increase risk")
    ...
    async def get_user_config_with_fallbacks(self, telegram_id: int, config_type: str) -> Dict[str, Any]:
    ...
    async def migrate_user_config(self, telegram_id: int, config_type: str,
                                migration_function) -> bool:
        ...
        try:
            ...
            is_valid, error_msg = await self.validate_config_comprehensive(config_type, migrated_config)
            if not is_valid:
                logger.error(f"Migration failed validation: {error_msg}")
                return False

            # Save migrated configuration
            return await self.set_user_config(telegram_id, config_type, migrated_config, validate=False)

        except Exception as e:
            logger.error(f"Error migrating user configuration: {e}")
            return False

    async def get_config_diff(self, telegram_id: int, config_type: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not user_config:
                return {"differences": {}, "message": "User has no custom configuration"}
