...
from sqlalchemy import Column, String, Boolean, DateTime, Integer, BigInteger, Text, JSON, ForeignKey, Enum
...
class TelegramUser(Base):
    ...
    subscription_status = Column(Enum(SubscriptionStatus), default=SubscriptionStatus.EXPIRED, nullable=False)
    ...
    signal_subscriptions = relationship("SignalSubscription", back_populates="user", cascade="all, delete-orphan")
...
class UserConfiguration(Base):
    ...
    config_type = Column(String(50), nullable=False)  # risk, symbol, signal, model, trading, rules
...
class PlatformConnection(Base):
    ...
    api_key = Column(String(255), nullable=False)  # For MT5: EA API key, For Crypto: API key
...
class SignalSubscription(Base):
    ...
    is_active = Column(Boolean, default=True, nullable=False)
    min_confidence = Column(Integer, default=60, nullable=False)  # Minimum confidence threshold
    created_at = Column(DateTime(timezone=True), server_default=func.now(), nullable=False)
    updated_at = Column(DateTime(timezone=True), server_default=func.now(), onupdate=func.now(), nullable=False)
