...
class AutoTradingCommandHandler(BaseCommandHandler):
    ...
    async def auto_trading_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        keyboard = create_keyboard([
            ...
            [("⚙️ Auto Signals", "auto_signals"), ("📋 View Settings", "view_auto_settings")],
    ...
    async def auto_signals_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Handle the /auto_signals command."""
        from src.core.config import config
        
        status = "🟢 ENABLED" if config.auto_trading.auto_signal_generation else "🔴 DISABLED"
        ...
        keyboard = create_keyboard([
            ...
            [("🤖 Auto Trading", "auto_trading"), ("📋 View Settings", "view_auto_settings")],
            [("✏️ Edit Pairs", "edit_auto_pairs"), ("📊 Status", "status")],
            [("🏠 Main Menu", "start")]
        ])
    ...
    async def toggle_auto_signals(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Toggle auto signal generation on/off."""
        from src.core.config import config
        
        # Toggle the setting
        config.auto_trading.auto_signal_generation = not config.auto_trading.auto_signal_generation
    ...
    async def view_auto_settings(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        keyboard = create_keyboard([
            [("🤖 Auto Trading", "auto_trading"), ("📡 Auto Signals", "auto_signals")],
            [("✏️ Edit Pairs", "edit_auto_pairs"), ("📊 Status", "status")],
            [("🏠 Main Menu", "start")]
        ])
