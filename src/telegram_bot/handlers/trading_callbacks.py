...
class TradingCallbackHandler:
    ...
    async def _handle_positions_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Handle positions callback."""
        await self.trading_handler.positions_command(update, context)

    async def _handle_orders_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Handle orders callback."""
        await self.trading_handler.orders_command(update, context)

    async def _handle_account_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Handle account callback."""
        await self.trading_handler.account_command(update, context)

    async def _handle_symbols_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Handle symbols callback."""
        await self.trading_handler.symbols_command(update, context)

    async def _handle_signals_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Handle signals callback."""
        await self.trading_handler.signals_command(update, context)

    async def _handle_position_details_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        keyboard = create_keyboard([
            [("🔄 Refresh", "refresh_positions"), ("⚡ Quick Close", "quick_close")],
    ...
    async def _handle_quick_close_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async def _handle_account_history_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        history_data = [
            {"date": "2024-01-15", "type": "Trade", "amount": 125.50, "balance": 10125.50},
            {"date": "2024-01-14", "type": "Trade", "amount": -45.30, "balance": 10000.00},
            {"date": "2024-01-13", "type": "Deposit", "amount": 1000.00, "balance": 10045.30},
            {"date": "2024-01-12", "type": "Trade", "amount": 67.80, "balance": 9045.30},
        ]
        
        history_text = ""
        for item in history_data:
            emoji = "💰" if item["type"] == "Deposit" else "💸" if item["amount"] < 0 else "💹"
            history_text += (
    ...
    async def _handle_live_dashboard_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async def _handle_webapp_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        keyboard = create_keyboard([
            ...
            [("📱 Mobile View", "webapp_mobile"), ("💻 Desktop View", "webapp_desktop")],
            [("🏠 Main Menu", "start")]
        ])

        await query.edit_message_text(message, reply_markup=keyboard, parse_mode="Markdown")

    async def _handle_signal_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if callback_data.startswith("signal_"):
            ...
            if len(parts) >= 3:
                ...
                signal = await self.trading_handler._generate_signal_for_pair(symbol, timeframe)
                ...
                keyboard = create_keyboard([
                    [("🔄 Refresh", f"signal_{symbol}_{timeframe}"), ("📊 All Signals", "signals")],
                    [("📈 Positions", "positions"), ("📋 Orders", "orders")],
                    [("🏠 Menu", "start")]
                ])
    ...
    async def _handle_export_history_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        keyboard = create_keyboard([
            ...
            [("📈 Export Excel", "export_excel"), ("📧 Email Report", "email_report")],
            [("📜 History", "account_history"), ("🏠 Menu", "start")]
        ])

        await query.edit_message_text(message, reply_markup=keyboard, parse_mode="Markdown")

    async def _handle_webapp_open_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async def _handle_webapp_mobile_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        keyboard = create_keyboard([
            [("📱 Open Mobile", "webapp_open"), ("💻 Desktop Version", "webapp_desktop")],
            [("🌐 WebApp Menu", "webapp"), ("🏠 Main Menu", "start")]
        ])

        await query.edit_message_text(message, reply_markup=keyboard, parse_mode="Markdown")

    async def _handle_webapp_desktop_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        keyboard = create_keyboard([
            [("💻 Open Desktop", "webapp_open"), ("📱 Mobile Version", "webapp_mobile")],
            [("🌐 WebApp Menu", "webapp"), ("🏠 Main Menu", "start")]
        ])
