...
class WebAppHandler:
    ...
    def create_webapp_keyboard(self, view_type: str = "trading"):
        ...
        keyboard = [
            [InlineKeyboardButton("🌐 Open Trading WebApp", web_app=WebAppInfo(url=webapp_url))],
            [
                InlineKeyboardButton("📊 Analytics", web_app=WebAppInfo(url=self.supported_views["analytics"])),
                InlineKeyboardButton("💼 Portfolio", web_app=WebAppInfo(url=self.supported_views["portfolio"]))
            ],
            [
                InlineKeyboardButton("⚙️ Settings", web_app=WebAppInfo(url=self.supported_views["settings"])),
        ...
        try:
            ...
            try:
                ...
                await update.message.reply_text(
                    "❌ Invalid data received from WebApp",
                    parse_mode="Markdown"
                )
            ...
            await update.message.reply_text(
                "❌ Error processing WebApp data",
                parse_mode="Markdown"
            )

    async def _process_webapp_data(self, update: Update, context: ContextTypes.DEFAULT_TYPE, data: Dict[str, Any]):
        ...
        if action == "place_order":
            ...
            await update.message.reply_text(
                f"❌ Unknown WebApp action: {action}",
                parse_mode="Markdown"
            )

    async def _handle_webapp_order(self, update: Update, context: ContextTypes.DEFAULT_TYPE, data: Dict[str, Any]):
    ...
    async def _handle_webapp_settings(self, update: Update, context: ContextTypes.DEFAULT_TYPE, data: Dict[str, Any]):
    ...
    async def _handle_webapp_data_request(self, update: Update, context: ContextTypes.DEFAULT_TYPE, data: Dict[str, Any]):
