...
class LiveDashboard:
    ...
    async def start_live_dashboard(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async def _show_loading_animation(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
...
def create_price_alert_keyboard(symbol: str, current_price: float) -> Dict[str, Any]:
    ...
    keyboard_buttons = [
        [("🔔 Enable Alert", f"alert_enable_{symbol}"), ("❌ Disable", f"alert_disable_{symbol}")],
        [("📈 Set High Alert", f"alert_high_{symbol}"), ("📉 Set Low Alert", f"alert_low_{symbol}")],
        [("⚙️ Custom Range", f"alert_custom_{symbol}"), ("🔄 Current Alerts", f"alert_list_{symbol}")]
    ]
