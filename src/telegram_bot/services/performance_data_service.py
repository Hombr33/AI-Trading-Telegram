...
class PerformanceDataService:
    ...
    async def get_performance_metrics(self, user_id: Optional[int] = None, days: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            avg_holding_time = total_holding_time / total_trades if total_trades > 0 else timedelta()
            ...
            monthly_profit = await self._calculate_monthly_profit(session, user_id, days)
            ...
            return {
                ...
                "avg_holding_time": str(avg_holding_time).split('.')[0]  # Remove microseconds
            }
    ...
    async def get_risk_metrics(self, user_id: Optional[int] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            # Calculate exposure
            total_exposure = sum(pos.volume * pos.current_price for pos in positions if pos.current_price)
            ...
            return {
                ...
                "daily_var": daily_var,
                "daily_var_pct": daily_var / 10000.0 if daily_var else 0,  # Assuming 10k account
                "margin_level": margin_level,
                "exposure": total_exposure / max_exposure if max_exposure > 0 else 0,
                "max_exposure": max_exposure,
                "largest_position": max((pos.volume * (pos.current_price or 0) for pos in positions), default=0),
                ...
                "risk_rating": self._calculate_risk_rating(drawdown, total_exposure, margin_level)
            }
    ...
    async def get_trading_journal(self, user_id: Optional[int] = None, limit: int = 50) -> List[Dict[str, Any]]:
    ...
    async def _calculate_daily_profit(self, session: Session, user_id: Optional[int], days: int) -> float:
    ...
    async def _calculate_weekly_profit(self, session: Session, user_id: Optional[int], days: int) -> float:
    ...
    async def _calculate_monthly_profit(self, session: Session, user_id: Optional[int], days: int) -> float:
    ...
    async def _count_trades_today(self, session: Session, user_id: Optional[int]) -> int:
    ...
    async def _count_trades_this_week(self, session: Session, user_id: Optional[int]) -> int:
    ...
    async def _count_trades_this_month(self, session: Session, user_id: Optional[int]) -> int:
    ...
    async def _calculate_drawdown(self, session: Session, user_id: Optional[int]) -> float:
    ...
    async def _calculate_max_drawdown(self, session: Session, user_id: Optional[int]) -> float:
    ...
    async def _calculate_daily_var(self, session: Session, user_id: Optional[int]) -> float:
    ...
    def _calculate_risk_rating(self, drawdown: float, exposure: float, margin_level: float) -> str:
