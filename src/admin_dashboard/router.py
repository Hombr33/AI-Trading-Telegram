    ...
    try:
        ...
        for user in all_users:
            if search:
                ...
                if not (search_term in user.get("username", "").lower() or
                       search_term in user.get("first_name", "").lower() or
                       search_term in user.get("last_name", "").lower() or
                       search_term in str(user.get("telegram_id", ""))):
                    continue
...
async def platform_management(
    ...
    try:
        ...
        for user in all_users:
            if user["is_active"]:
                connections = await service.user_manager.get_user_platform_connections(user["telegram_id"])
                for conn in connections:
                    platform_connections.append({
                        "user": user,
                        "connection": conn
                    })
