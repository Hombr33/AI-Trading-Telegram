...
class MT5BridgeService:
    ...
    def _setup_event_handlers(self):
        ...
        asyncio.create_task(self.socketio_bridge.on_event("order", self._handle_order_event))
        asyncio.create_task(self.socketio_bridge.on_event("signal", self._handle_signal_event))
        asyncio.create_task(self.socketio_bridge.on_event("position_update", self._handle_position_update))
        asyncio.create_task(self.socketio_bridge.on_event("risk_alert", self._handle_risk_alert))
    ...
    async def get_positions_for_telegram(self, telegram_id: int) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if positions:
                logger.info(f"Retrieved {len(positions)} positions from EA for user {telegram_id}")
                return positions
            
            # Fallback to trading data service
            positions = await self.trading_data_service.get_positions()
            logger.info(f"Retrieved {len(positions)} positions from trading data service")
            return positions
    ...
    async def get_signals_for_telegram(self, telegram_id: int, limit: int = 10) -> List[Dict[str, Any]]:
    ...
    async def send_signal_to_ea(self, signal_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Signal sent via Socket.IO Bridge: {signal_data.get('signal_id')}")
            return result
    ...
    async def modify_position(self, telegram_id: int, position_ticket: int, 
                            new_sl: float = None, new_tp: float = None) -> bool:
        """Modify position in EA."""
        try:
            success = await self.ea_bridge.modify_position_in_ea(telegram_id, position_ticket, new_sl, new_tp)
            if success:
    ...
    async def close_position(self, telegram_id: int, position_ticket: int, 
                           volume: float = None) -> bool:
        """Close position in EA."""
        try:
            success = await self.ea_bridge.close_position_in_ea(telegram_id, position_ticket, volume)
            if success:
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        status = {
            ...
            "socketio_bridge": "connected" if self.socketio_bridge.connected else "disconnected",
            "mt5_connection": "available" if self.trading_data_service.is_mt5_available() else "unavailable",
