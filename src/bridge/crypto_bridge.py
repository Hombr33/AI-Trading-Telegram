...
class CryptoBridge:
    ...
    async def register_crypto_connection(
        self,
        telegram_id: int,
        exchange: str,
        api_key: str,
        ...
        try:
            ...
            if not await self.validate_crypto_credentials(
            ...
            if success:
                logger.info(
                    f"Crypto connection registered for user {telegram_id}: {exchange}"
                )
                return True
    ...
    async def validate_crypto_credentials(
        self, exchange: str, api_key: str, api_secret: str, testnet: bool = False
    ) -> bool:
        """Validate crypto exchange API credentials."""
        try:
            if exchange == "binance":
                return await self._validate_binance_credentials(
    ...
    async def get_crypto_account_info(
        ...
        try:
            with get_db_session() as session:
                ...
                exchange = (
                    connection.server_endpoint.split("_")[0]
                    if connection.server_endpoint
                    else "binance"
                )
                testnet = (
                    "_testnet" in connection.server_endpoint
                    if connection.server_endpoint
                    else False
                )

                if exchange == "binance":
                    return await self._get_binance_account_info(
                        connection.api_key, connection.api_secret, testnet
                    )
                elif exchange == "bybit":
                    return await self._get_bybit_account_info(
                        connection.api_key, connection.api_secret, testnet
                    )
    ...
    async def _get_binance_account_info(
        self, api_key: str, api_secret: str, testnet: bool
    ) -> Dict[str, Any]:
    ...
    async def _get_bybit_account_info(
        self, api_key: str, api_secret: str, testnet: bool
    ) -> Dict[str, Any]:
    ...
    async def _place_binance_order(
        self, api_key: str, api_secret: str, order_data: Dict[str, Any], testnet: bool
    ) -> Dict[str, Any]:
        ...
        async with ClientSession(timeout=self.timeout) as session:
            async with session.post(
                f"{base_url}{endpoint}", data=params, headers=headers
            ) as response:
                if response.status == 200:
    ...
    async def get_crypto_positions(
        ...
        try:
            with get_db_session() as session:
                ...
                if exchange == "binance":
                    return await self._get_binance_open_orders(
                        connection.api_key, connection.api_secret, testnet
                    )
                elif exchange == "bybit":
                    return await self._get_bybit_open_orders(
                        connection.api_key, connection.api_secret, testnet
                    )
    ...
    async def _get_binance_open_orders(
        self, api_key: str, api_secret: str, testnet: bool
    ) -> List[Dict[str, Any]]:
    ...
    async def _get_bybit_open_orders(
        self, api_key: str, api_secret: str, testnet: bool
    ) -> List[Dict[str, Any]]:
