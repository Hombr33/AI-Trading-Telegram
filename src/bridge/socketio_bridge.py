...
class SocketIOBridge:
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            logger.warning(
                f"Socket.IO bridge setup failed: {e}, enabling fallback mode"
            )
            self.fallback_enabled = True
            return False
    ...
    async def send_signal(self, signal_data: Dict) -> Dict:
        ...
        try:
            if self.connected:
                ...
                logger.info(
                    f"Signal sent via Socket.IO: {signal_data.get('signal_id')}"
                )
                return {"success": True, "method": "socketio"}
            else:
                # Fallback to HTTP
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    if response.status == 200:
                        ...
                        logger.info(
                            f"Order sent via HTTP fallback: {order_data.get('order_id')}"
                        )
                        return {
                            "success": True,
                        ...
                        logger.error(
                            f"HTTP fallback order send failed: {response.status} - {error_text}"
                        )
                        return {
                            "success": False,
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    if response.status == 200:
                        result = await response.json()
                        logger.info(
                            f"Signal sent via HTTP fallback: {signal_data.get('signal_id')}"
                        )
                        return {
                            "success": True,
                            "method": "http_fallback",
                        ...
                        error_text = await response.text()
                        logger.error(
                            f"HTTP fallback signal send failed: {response.status} - {error_text}"
                        )
                        return {
                            "success": False,
                            "error": f"HTTP {response.status}: {error_text}",
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    if response.status == 200:
                        ...
                        error_text = await response.text()
                        logger.error(
                            f"HTTP fallback position update failed: {response.status} - {error_text}"
                        )
                        return {
                            "success": False,
                            "error": f"HTTP {response.status}: {error_text}",
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    if response.status == 200:
                        ...
                        error_text = await response.text()
                        logger.error(
                            f"HTTP fallback risk alert failed: {response.status} - {error_text}"
                        )
                        return {
                            "success": False,
                            "error": f"HTTP {response.status}: {error_text}",
